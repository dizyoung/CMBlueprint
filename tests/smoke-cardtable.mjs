...
const summary = await page.$eval('.stack-summary', (el) => el.textContent).catch(() => '');
ok('The stack shows Together', summary.includes('Together'));
ok('The stack names both Forms', summary.includes('Form I') && summary.includes('Form II'));
ok('The stack shows a loop status', summary.includes('No loop yet'));
...
const afterSummary = await page.$eval('.stack-summary', (el) => el.textContent).catch(() => '');
ok('The reloaded stack still reads Together', afterSummary.includes('Together'));

// --- the rest of the success path -----------------------------------------
await page.click('.stack-summary button[data-action="open"]');
await page.waitForTimeout(200);
