...
// Return all card IDs that are "covered" by a loop that's placed in the rhythm.
// A loop item covers a card if loopItem.linkedCardId === cardId.
export function getLoopCoveredCardIds(rhythm, loops, loopItems) {
  if (!rhythm || !rhythm.assignments) return [];
  ...
  (loopItems || []).forEach(function(item) {
    if (item.linkedCardId && placedLoopIds.indexOf(item.loopId) > -1) {
      if (coveredCardIds.indexOf(item.linkedCardId) === -1) {
        coveredCardIds.push(item.linkedCardId);
      }
    }
  });
  return coveredCardIds;
}
...
// Compute a unified placement summary: which cards/loops/sequences are placed,
// which are covered by a placed loop (via linkedCardId), and which are not placed.
// opts.includeStatuses (default ['active']) controls which card statuses are considered.
export function getPlacementSummary(rhythm, cards, loops, sequences, loopItems, opts) {
  opts = opts || {};
  var statuses = opts.includeStatuses || ['active'];
  if (!rhythm || !rhythm.assignments) {
    return {
      placedCardIds: [],
      placedLoopIds: [],
      placedSequenceIds: [],
      coveredCardIds: [],
      notPlacedCards: (cards || []).filter(function(c) { return statuses.indexOf(c.status) > -1; }),
      notPlacedLoops: (loops || []).slice(),
      notPlacedSequences: (sequences || []).slice()
    };
  }
  var placedCardIds = [];
  var placedLoopIds = [];
  var placedSequenceIds = [];
  (rhythm.assignments || []).forEach(function(a) {
    if (a.assignmentType === 'card' && a.referencedId && placedCardIds.indexOf(a.referencedId) === -1)
      placedCardIds.push(a.referencedId);
    if (a.assignmentType === 'loop' && a.referencedId && placedLoopIds.indexOf(a.referencedId) === -1)
      placedLoopIds.push(a.referencedId);
    if (a.assignmentType === 'sequence' && a.referencedId && placedSequenceIds.indexOf(a.referencedId) === -1)
      placedSequenceIds.push(a.referencedId);
  });
  var coveredCardIds = getLoopCoveredCardIds(rhythm, loops, loopItems);
  var notPlacedCards = (cards || []).filter(function(c) {
    return statuses.indexOf(c.status) > -1 &&
      placedCardIds.indexOf(c.id) === -1 &&
      coveredCardIds.indexOf(c.id) === -1;
  });
  var notPlacedLoops = (loops || []).filter(function(l) { return placedLoopIds.indexOf(l.id) === -1; });
  var notPlacedSequences = (sequences || []).filter(function(s) { return placedSequenceIds.indexOf(s.id) === -1; });
  return {
    placedCardIds: placedCardIds,
    placedLoopIds: placedLoopIds,
    placedSequenceIds: placedSequenceIds,
    coveredCardIds: coveredCardIds,
    notPlacedCards: notPlacedCards,
    notPlacedLoops: notPlacedLoops,
    notPlacedSequences: notPlacedSequences
  };
}
