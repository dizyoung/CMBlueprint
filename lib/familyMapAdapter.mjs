...
export function buildFeastRows(state) {
  ...
  return (M.FEAST_LIBRARY || []).map(function (strand) {
    ...
    } else if (mode === 'loop' && loopId && loopHasRhythmPlacement(state, loopId)) {
    ...
    } else if (mode && cardsForStrandAssignment(state, assignment).some(function (c) { return cardHasRhythmPlacement(state, c.id); })) {
...
export function setStrandAssignmentForStrand(state, strandId, strandLabel, fields) {
  ...
  var base = {
    loopId: null, groupId: null, studentIds: [], coopProvider: ''
  };
  if (existing) {
...
// A day is AVAILABLE when the student's own workdays[day] is true AND no
// commitment they participate in lists that weekday with blocksRegularWork.
export function getStudentAvailability(state, studentId) {
  ...
  var blockedBy = {};
  var coopOnly = {};
  days.forEach(function (d) { blockedBy[d] = []; coopOnly[d] = false; });
  ...
  setupCommitments(state).forEach(function (c) {
    ...
    sArr(c.weekdays).forEach(function (d) {
      ...
      if (c.allowsCoopWork !== false) coopOnly[d] = true;
    });
  });

  var availableDayIds = days.filter(function (d) { return workdays[d] && blockedBy[d].length === 0; });
  var coopOnlyDayIds = days.filter(function (d) { return workdays[d] && blockedBy[d].length > 0 && coopOnly[d]; });
...
export function getGroupAvailability(state, groupId) {
  ...
  var actives = getEveryoneStudentIds(state);
  var memberIds = grp ? sArr(grp.studentIds).filter(function (id) { return actives.indexOf(id) > -1; }) : [];
  if (!memberIds.length) {
    return { groupId: groupId, memberIds: [], availableDayIds: [], partialDayIds: [] };
  }
  var perMember = memberIds.map(function (id) { return getStudentAvailability(state, id).availableDayIds; });
  var availableDayIds = days.filter(function (d) {
    return perMember.every(function (list) { return list.indexOf(d) > -1; });
  });
  var partialDayIds = days.filter(function (d) {
    var some = perMember.some(function (list) { return list.indexOf(d) > -1; });
    return some && availableDayIds.indexOf(d) === -1;
  });
  return { groupId: groupId, memberIds: memberIds, availableDayIds: availableDayIds, partialDayIds: partialDayIds };
}
...
export function getSetupProgress(state) {
  ...
  var rhythmComplete = ack.rhythmReviewed === true;
  var rhythmDetail = rhythmComplete
    ? 'Loops and broad rhythm confirmed.'
    : 'Not confirmed yet — say "These look right" on the Rhythm step.';
