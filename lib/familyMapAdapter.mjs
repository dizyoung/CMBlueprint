...
//
// setStrandAssignmentForStrand answers "who is this strand FOR", and clearing
// audience there legitimately drops a prototype-created record. Loop sorting is
// a different question: moving a strand out of a loop must never throw away the
// unrelated choices already made about it (work type, the light-day opt-in,
// notes). So this mover NEVER deletes a record — it only ever rewrites the two
// fields that describe loop membership.
//
//  - targetLoopId non-null -> assignmentMode 'loop', loopId set
//  - targetLoopId null     -> back to Unsorted: assignmentMode null, loopId null,
//                             record KEPT so workType / mayOccurOnLightDays /
//                             notes survive
//
...
export function moveStrandLoopAssignment(state, strandId, strandLabel, targetLoopId) {
  ...
  if (existing) {
    existing.assignmentMode = loopId ? 'loop' : null;
    existing.loopId = loopId;
    // Moving into a loop is an audience change away from any group/individual
    // answer, so those fields cannot linger — but workType, mayOccurOnLightDays,
    // notes, sortOrder, createdBy, and id are untouched on purpose.
    existing.groupId = null;
    existing.studentIds = [];
    existing.coopProvider = '';
