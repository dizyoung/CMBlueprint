...
export function buildSetupSummary(state) {
  var family = activeStudents(state).map(function (s) {
    return {
      ...
      availableDayIds: fullCapacityDayIds(state, s.id)
    };
  });
...
export function getLoopSortingProgress(state) {
  ...
  return {
    ...
    setupStepComplete: (bucketsDefined || noLoopsChosen) && bucketsReviewed,
...
export function getSetupProgress(state) {
  ...
  var withDays = actives.filter(function (s) { return fullCapacityDayIds(state, s.id).length > 0; });
  ...
  if (!sorting.bucketsDefined && !sorting.noLoopsChosen) {
