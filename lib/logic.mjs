...
// ---------------------------------------------------------------------------
// View 2: AO-style Term Chart — "where the plan says we are going". Rows =
...
// ---------------------------------------------------------------------------
export function buildAOTermChart(categories, children, schoolYear, term, filter, opts) {
  opts = opts || {};
  var ranges = termRanges(schoolYear);
  var range = ranges[term];
...
// Returns { requiredSetup, scheduling, content, optional } — each an array of
// { type, severity, text, grpName, catName, catId, bookKey, status, actions }.
// `actions` is a list of action keys the UI can offer as quick-action buttons.
export function buildNeedsDecisions(categories, familySetup) {
  var out = { requiredSetup: [], scheduling: [], content: [], optional: [] };

  if (!familySetup || !familySetup.children || familySetup.children.length === 0) {
    out.requiredSetup.push({ type: 'requiredSetup', text: 'Add at least one child to get started.', actions: ['goSetup'] });
  }
  if (!categories || categories.length === 0) {
    out.requiredSetup.push({ type: 'requiredSetup', text: 'Add subject categories to plan your year.', actions: ['goCats'] });
  }

  findBooksNeedingLoopAssignment(categories || []).forEach(function (n) {
    out.scheduling.push({
      type: 'scheduling', text: n.grpName + ' — ' + n.catName + ': "' + n.title + '" needs a loop assigned.',
  ...
  (categories || []).forEach(function (grp) {
    grp.items.forEach(function (cat) {
      ...
      var entry = {
        type: 'content', text: grp.grp + ' — ' + cat.name + ' has no books assigned yet.',
        grpName: grp.grp, catName: cat.name, catId: cat.id, status: status,
        actions: ['addBook', 'markPractice', 'markManual', 'markCoop', 'ignoreYear']
      };
      if (status === 'active') out.content.push(entry);
      else if (status !== 'ignored') out.optional.push(Object.assign({}, entry, {
        text: grp.grp + ' — ' + cat.name + ': ' + DECISION_STATUS_LABELS[status]
      }));
      // status === 'ignored' -> excluded entirely, by design.
    });
  });
...
export function countUrgentDecisions(decisions) {
  return decisions.requiredSetup.length + decisions.scheduling.length + decisions.content.length;
}
