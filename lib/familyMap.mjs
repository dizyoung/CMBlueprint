...
export function makeLoop(fields) {
  return Object.assign({
    ...
    active: true,
    // ADDITIVE: a short optional description the parent may give a bucket in
    // Setup. Part of the bucket's identity, so it takes part in the fingerprint.
    description: '',
    // ADDITIVE: an audience a bucket may PREFILL onto a strand that has none at
    // the moment it joins. It never overwrites an audience a strand already has,
    // and it is never applied retroactively. null = no default.
    defaultAudience: null,
    // Broad weekday hints. KEPT in the model (never deleted, existing values
    // preserved) but no longer asked for in Setup — actual day placement is a
    // Weekly Rhythm decision, so this is not part of the bucket fingerprint.
    rhythmDayIds: [],
    subjectColumnId: null,
    audience: 'together',
...
// AUDIENCE ONLY. `assignmentMode` answers "who is this strand for" and nothing
// else. Loop membership is a SEPARATE, orthogonal field (`loopId`): a strand may
// have an audience and no loop, a loop and no audience, both, or neither.
//
// 'loop' is NOT in this list any more — it was never an audience, it was a
// container, and storing it here destroyed the audience whenever a strand joined
// a loop. It is still ACCEPTED ON READ for legacy saved plans (see
// LEGACY_STRAND_ASSIGNMENT_MODES / resolveStrandAudience), and
// migrateSetupPrototypeState rewrites it once, keeping the loopId.
...
// Values that may still appear in saved plans but are never written any more.
export const LEGACY_STRAND_ASSIGNMENT_MODES = ['loop'];

// Everything a stored assignmentMode is allowed to be when READ.
export const STRAND_ASSIGNMENT_MODES_ACCEPTED_ON_READ =
  STRAND_ASSIGNMENT_MODES.concat(LEGACY_STRAND_ASSIGNMENT_MODES);

export function isLegacyLoopAssignmentMode(mode) {
  return mode === 'loop';
}

// The AUDIENCE actually in force for an assignment. Legacy 'loop' means the
// audience was never recorded — it is genuinely unresolved, and we never invent
// one. Returns null when there is no audience.
export function resolveStrandAudience(assignment) {
  var a = (assignment && typeof assignment === 'object') ? assignment : {};
  var mode = a.assignmentMode;
  if (!mode || isLegacyLoopAssignmentMode(mode)) return null;
  return STRAND_ASSIGNMENT_MODES.indexOf(mode) > -1 ? mode : null;
}

// Is this strand part of this year's plan? ORTHOGONAL to audience.
//
// The stored field is `useThisYear`. Legacy plans recorded the same decision
// by overloading the audience field with 'not-this-year' — that is still
// ACCEPTED ON READ (and rewritten once by migrateSetupPrototypeState), but it is
// never written any more, because writing it destroyed the audience.
// No assignment at all means the strand is simply undecided, which is active.
export function resolveStrandActiveThisYear(assignment) {
  var a = (assignment && typeof assignment === 'object') ? assignment : null;
  if (!a) return true;
  if (a.useThisYear === false) return false;
  if (a.useThisYear === true) return true;
  return a.assignmentMode !== 'not-this-year';
}

// The LOOP a strand sits in, independent of its audience.
export function resolveStrandLoopId(assignment) {
  var a = (assignment && typeof assignment === 'object') ? assignment : {};
  return a.loopId == null ? null : a.loopId;
}
...
export function makeStrandAssignment(fields) {
  return Object.assign({
    ...
    assignmentMode: 'everyone',  // AUDIENCE only — see STRAND_ASSIGNMENT_MODES
    loopId: null,            // ORTHOGONAL to audience: which loop holds this strand
    ...
    workType: null,
    // ADDITIVE: the parent's "use this strand this year" decision. ORTHOGONAL
    // to audience, exactly like loopId. Turning a strand off must never clear
    // who it is for, which loop holds it, its work type, or its notes.
    // Stored. The feast row's activeThisYear is DERIVED from this, never stored.
    useThisYear: true,
...
export function deriveRequiredCards(strandAssignments, students, groups, loops) {
  ...
  (strandAssignments || []).forEach(function(sa) {
    // Off for this year, however that was recorded (new flag or legacy mode).
    if (!resolveStrandActiveThisYear(sa)) return;
    if (sa.assignmentMode === 'loop') return;
    // A strand inside a loop is covered by that loop's own placement — it does
    // not also produce a separate weekly card, whatever its audience says.
    if (resolveStrandLoopId(sa)) return;
...
// ---------------------------------------------------------------------------
// Feast stacks — PRESENTATION ONLY.
//
// A stack is a VIEW over FEAST_LIBRARY by stable strand id. Stacks never merge
// records: a bulk action on a stack applies to each of its strands individually,
// and any one strand may override afterwards. Every strand belongs to exactly
// one stack (first match wins, with a per-column fallback), so the default view
// is complete — nothing can hide in a gap between stacks.
// ---------------------------------------------------------------------------
function labelHas(entry, needle) {
  return String((entry && entry.label) || '').toLowerCase().indexOf(String(needle).toLowerCase()) > -1;
}
function labelHasAny(entry, needles) {
  return needles.some(function (n) { return labelHas(entry, n); });
}

export const FEAST_STACKS = [
  { id: 'narration', label: 'Narration', match: function (e) { return labelHas(e, 'Narration'); } },
  {
    id: 'natural-history', label: 'Natural History & Nature',
    match: function (e) { return labelHasAny(e, ['Natural History', 'Nature Walk', 'Object Lessons']); }
  },
  { id: 'science', label: 'Science', match: function (e) { return e.column === 'science'; } },
  {
    id: 'citizenship', label: 'Citizenship & Current Events',
    match: function (e) {
      return e.column === 'geography' && labelHasAny(e, ['Citizenship', 'Plutarch', 'Current Events', 'Government']);
    }
  },
  { id: 'geography', label: 'Geography & Map Work', match: function (e) { return e.column === 'geography'; } },
  { id: 'history', label: 'History', match: function (e) { return e.column === 'history'; } },
  { id: 'literature', label: 'Literature', match: function (e) { return e.column === 'literature'; } },
  { id: 'beauty', label: 'Beauty & Handicraft', match: function (e) { return e.column === 'beauty'; } },
  {
    id: 'grammar-composition', label: 'Grammar & Composition',
    match: function (e) { return e.column === 'language-arts' && labelHasAny(e, ['Grammar', 'Composition']); }
  },
  { id: 'early-reading', label: 'Early Reading & Copywork', match: function (e) { return e.column === 'language-arts'; } },
  { id: 'bible', label: 'Bible', match: function (e) { return e.column === 'bible'; } },
  { id: 'math', label: 'Math', match: function (e) { return e.column === 'math'; } },
  { id: 'languages', label: 'Languages', match: function (e) { return e.column === 'languages'; } }
];

// Auto stack id for anything no rule claims, so coverage is always total.
export function autoStackIdForColumn(columnId) {
  return 'column:' + String(columnId == null ? 'other' : columnId);
}

export function stackForStrand(entry) {
  var e = (entry && typeof entry === 'object') ? entry : {};
  for (var i = 0; i < FEAST_STACKS.length; i++) {
    var stack = FEAST_STACKS[i];
    var hit = false;
    try { hit = stack.match(e) === true; } catch (err) { hit = false; }
    if (hit) return stack.id;
  }
  return autoStackIdForColumn(e.column);
}

export function stackLabel(stackId) {
  var s = FEAST_STACKS.find(function (x) { return x.id === stackId; });
  if (s) return s.label;
  if (String(stackId).indexOf('column:') === 0) {
    var colId = String(stackId).slice('column:'.length);
    var col = DEFAULT_VISIBLE_SUBJECT_COLUMNS.find(function (c) { return c.id === colId; });
    return col ? col.label : colId;
  }
  return String(stackId == null ? '' : stackId);
}

// [{ id, label, strandIds }] in FEAST_STACKS order, then any auto stacks in the
// order their first member appears. Empty stacks are omitted.
export function buildFeastStacks() {
  var byId = {};
  var order = [];
  (FEAST_LIBRARY || []).forEach(function (entry) {
    var id = stackForStrand(entry);
    if (!byId[id]) { byId[id] = { id: id, label: stackLabel(id), strandIds: [] }; order.push(id); }
    byId[id].strandIds.push(entry.id);
  });
  var ranked = order.slice().sort(function (a, b) {
    var ia = FEAST_STACKS.findIndex(function (s) { return s.id === a; });
    var ib = FEAST_STACKS.findIndex(function (s) { return s.id === b; });
    if (ia === -1) ia = FEAST_STACKS.length + order.indexOf(a);
    if (ib === -1) ib = FEAST_STACKS.length + order.indexOf(b);
    return ia - ib;
  });
  return ranked.map(function (id) { return byId[id]; });
}
