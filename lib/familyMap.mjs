...
// opts.mayOccurOnLightDays is a STRAND-level opt-in: it flips the whole
// light-independent column to allowed for that one strand. 'off' is never
// overridable. Reasons are parent-facing and rendered as-is.
export function workTypeAllowedOnCapacity(workTypeId, capacityId, opts) {
  ...
  if (workTypeId === 'group-lesson') {
    if (optIn) {
      return { allowed: true, reason: 'Allowed on a light day because this strand opts in — normally group lessons are not.' };
    }
    return { allowed: false, reason: 'A light day is for independent work, so the group does not meet.' };
  }
  // shared-with-mom
  if (optIn) {
    return { allowed: true, reason: 'Allowed on a light day because this strand opts in — normally work shared with Mom is not.' };
  }
